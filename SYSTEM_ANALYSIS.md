...
2.  **Daily Operations (Morning Ritual):**
    ...
    - _System Action:_ Automatically creates 500+ `SCHEDULED` orders for all customers expecting delivery today.
...
3.  **Assignment & Dispatch:**
    ...
    - _Result:_ Orders move to `PENDING` and appear instantly on Ali's phone.
...
1.  **Start of Day:**
    ...
    - Views the **Load Sheet**: _"Load 40x 19L Bottles, 10x 1.5L Cartons"_.
...
- **Order <-> Ledger:** One-to-one financial record.
  - _Sale:_ Debits customer balance.
  - _Payment:_ Credits customer balance.
- **Order <-> Inventory:**
  - _Completion:_ Decrements `Stock Filled`, Increments `Stock Empty`.
  - _Bottle Wallet:_ Updates customer's holding of empty bottles.

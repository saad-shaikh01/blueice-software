...
export const getFileIcon = (type: string): FileIconElement => {
  ...
  if (
    ...
    return <FileArchive className="h-9 w-9 text-yellow-600 dark:text-yellow-500" />;
  }

  // Text files
  if (type === 'text/plain' || type === 'text/markdown') {
    return <FileText className="h-9 w-9 text-gray-500 dark:text-gray-400" />;
  }

  // JSON/XML files
  if (type === 'application/json' || type === 'application/xml') {
    return <FileJson className="h-9 w-9 text-orange-600 dark:text-orange-500" />;
  }

  // Code files
  if (
    type.startsWith('text/javascript') ||
    type.startsWith('text/typescript') ||
    type.startsWith('text/css') ||
    type.startsWith('text/html') ||
    type === 'application/javascript' ||
    type === 'application/typescript'
  ) {
    return <FileCode className="h-9 w-9 text-indigo-600 dark:text-indigo-400" />;
  }

  // Default file icon
  return <File className="h-9 w-9 text-gray-500 dark:text-gray-400" />;
};
